...
def get_orchestrator() -> TeamOrchestrator:
    """TeamOrchestrator 싱글톤 인스턴스를 반환합니다.

    Returns:
        TeamOrchestrator 인스턴스
    """
...
async def upload_team_jsonl(
    file: UploadFile = File(..., description="업로드할 팀 JSONL 파일"),
) -> JSONResponse:
    """팀 JSONL 파일을 업로드하고 첫 5개 행을 출력합니다.
    ...
    """
    logger.info(f"[팀 업로드 시작] 파일명: {file.filename if file.filename else 'Unknown'}")

    # 파일 확장자 검증
    if not file.filename or not file.filename.endswith('.jsonl'):
        logger.warning(f"[팀 업로드 실패] 잘못된 파일 형식: {file.filename}")
        raise HTTPException(
            status_code=400,
    ...
    try:
        # 파일 내용 읽기
        logger.info("[팀 업로드] 파일 읽기 시작...")
        contents = await file.read()
        logger.info(f"[팀 업로드] 파일 크기: {len(contents)} bytes")
        text_content = contents.decode('utf-8')
        logger.info("[팀 업로드] 파일 디코딩 완료")
        ...
        processing_result = await orchestrator.process_teams(items)
        ...
        logger.error(f"[팀 업로드 오류] {str(e)}", exc_info=True)
        raise HTTPException(
            status_code=500,
