...
class PlayerOrchestrator:
    ...
    def __init__(
        ...
        # FastMCP 서버 생성
        self.mcp = FastMCP(name="player_orchestrator_koelectra")
        ...
        logger.info("[오케스트레이터] PlayerOrchestrator 초기화 완료 (LangGraph, KoELECTRA, ExaOne, FastMCP)")
    ...
    def _setup_koelectra_tools(self) -> None:
        ...
        def koelectra_classify_text(text: str) -> Dict[str, Any]:
            ...
            try:
                ...
                with torch.no_grad():
                    outputs = self.koelectra_model(**inputs)
                    # [CLS] 토큰의 임베딩을 사용하여 분류 (실제로는 분류 헤드가 필요하지만 예시로 제공)
    ...
    def _setup_integrated_tools(self) -> None:
        ...
        async def koelectra_to_exaone_pipeline(text: str) -> Dict[str, Any]:
            ...
            try:
                ...
                return {
                    ...
                    "koelectra_embedding": {
                        "dimension": len(embedding),
                        "sample": embedding[:10]  # 처음 10개만 샘플
                    },
        ...
        async def analyze_player_with_models(player_data: Dict[str, Any]) -> Dict[str, Any]:
            ...
            Args:
                player_data: 분석할 선수 데이터 딕셔너리

            Returns:
                종합 분석 결과 딕셔너리
            """
            try:
                logger.info(f"[통합 분석] 선수 데이터 분석 시작: {player_data.get('player_name', 'Unknown')}")
                ...
                logger.info("[통합 분석] 선수 데이터 분석 완료")
                ...
                logger.error(f"[통합 분석] 선수 데이터 분석 실패: {e}", exc_info=True)
                return {
                    "success": False,
                    "error": str(e),
                    "player_data": player_data
