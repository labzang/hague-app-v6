...
class ScheduleOrchestrator:
    ...
    def __init__(
        ...
        # FastMCP 서버 생성
        self.mcp = FastMCP(name="schedule_orchestrator_koelectra")
        ...
        logger.info("[오케스트레이터] ScheduleOrchestrator 초기화 완료 (LangGraph, KoELECTRA, ExaOne, FastMCP)")
    ...
    async def _policy_process_node(self, state: ScheduleProcessingState) -> Dict[str, Any]:
        ...
        try:
            result = await self.agent.process_schedules(items)
    ...
    async def process_query(self, question: str) -> Dict[str, Any]:
        ...
        Returns:
            처리 결과 딕셔너리
        """
        logger.info(f"[ScheduleOrchestrator] 질문 수신: {question}")
        print(f"[ScheduleOrchestrator] 사용자 질문: {question}")
        ...
        logger.info(f"[ScheduleOrchestrator] 질문 처리 완료: {question}")
        return result
