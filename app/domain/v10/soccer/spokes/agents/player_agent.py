...
class PlayerAgent:
    ...
    def __init__(self, model_dir: Optional[Path] = None):
        ...
        self.mcp = FastMCP(name="player_agent_exaone")
        self._setup_exaone_tools()

        logger.info("[에이전트] PlayerAgent 초기화 완료 (ExaOne, FastMCP)")
    ...
    def _setup_exaone_tools(self) -> None:
        ...
        def exaone_analyze_player_data(player_data: Dict[str, Any]) -> Dict[str, Any]:
            ...
            try:
                ...
                logger.info("[ExaOne 툴] 선수 데이터 분석 완료")
                return {
                    "success": True,
                    "analysis": response,
                ...
                logger.error(f"[ExaOne 툴] 선수 데이터 분석 실패: {e}", exc_info=True)
                return {
                    "success": False,
                    "error": str(e)
                }
