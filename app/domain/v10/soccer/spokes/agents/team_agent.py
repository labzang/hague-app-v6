...
class TeamAgent:
    ...
    def __init__(self, model_dir: Optional[Path] = None):
        ...
        """
        logger.info("[에이전트] TeamAgent 초기화")

        # ExaOne 모델 로드
        self.exaone_llm = self._load_exaone_model(model_dir)

        # FastMCP 클라이언트 생성 및 툴 설정
        self.mcp = FastMCP(name="team_agent_exaone")
        self._setup_exaone_tools()

        logger.info("[에이전트] TeamAgent 초기화 완료 (ExaOne, FastMCP)")
    ...
    def _setup_exaone_tools(self) -> None:
        ...
        def exaone_analyze_team_data(team_data: Dict[str, Any]) -> Dict[str, Any]:
            ...
            try:
                ...
                logger.info("[ExaOne 툴] 팀 데이터 분석 완료")
                return {
                    "success": True,
                    "analysis": response,
                    "team_data": team_data
                ...
                logger.error(f"[ExaOne 툴] 팀 데이터 분석 실패: {e}", exc_info=True)
                return {
                    "success": False,
                    "error": str(e)
                }
