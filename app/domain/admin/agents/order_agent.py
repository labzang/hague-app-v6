...
class OrderAgent(BaseAgent):
    ...
    def __init__(
        ...
        name: str = "OrderAgent",
        instruction: str = "주문 관련 작업을 정책 기반으로 처리합니다."
    ):
        """OrderAgent 초기화.
    ...
    async def execute(self, task: str, context: Dict[str, Any]) -> Dict[str, Any]:
        ...
        Args:
            task: 실행할 작업 설명
            context: 실행 컨텍스트 (action, data, order_id 등)
        ...
        if self.model is None or self.tokenizer is None:
            ...
            return await self._fallback_execute(action, data, order_id)

        # 정책 기반 처리 (모델 사용)
        try:
            # 프롬프트 생성
            prompt = self._create_prompt(action, data, order_id)
            ...
            # 응답 파싱 및 결과 생성
            result = self._parse_response(action, response, data, order_id)
    ...
    def _create_prompt(self, action: str, data: Dict[str, Any], order_id: Optional[int]) -> str:
        """작업에 맞는 프롬프트 생성."""
        base_prompt = f"""주문 관리 시스템에서 다음 작업을 수행하세요:

작업: {action}
"""

        if order_id:
            base_prompt += f"주문 ID: {order_id}\n"

        if data:
            base_prompt += f"데이터: {data}\n"

        if action == "cancel":
            base_prompt += "\n주문 취소 정책을 고려하여 적절한 처리를 수행하세요."
        elif action == "create":
            base_prompt += "\n주문 생성 정책을 고려하여 적절한 처리를 수행하세요."
        else:
