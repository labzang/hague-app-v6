...
from app.domain.admin.services.consumer_service import ConsumerService
from app.domain.admin.agents.consumer_agent import ConsumerAgent
...
class ConsumerFlow:
    ...
    async def process_request(
        ...
        Args:
            action: 수행할 액션 (create, update, get, list, delete)
            data: 요청 데이터
            consumer_id: 소비자 ID
        ...
        if use_policy:
            # 정책 기반 처리 (Agent)
            return await self._process_with_agent(action, data, consumer_id)
        else:
            # 규칙 기반 처리 (Service)
            return await self._process_with_service(action, data, consumer_id)
    ...
    async def _process_with_service(
        ...
        try:
            if action == "create":
                return await self.service.create_consumer(data or {})
            elif action == "update":
                if consumer_id is None:
                    raise ValueError("consumer_id가 필요합니다")
                return await self.service.update_consumer(consumer_id, data or {})
            elif action == "get":
                if consumer_id is None:
                    raise ValueError("consumer_id가 필요합니다")
                return await self.service.get_consumer(consumer_id)
            elif action == "list":
                limit = data.get("limit", 100) if data else 100
                offset = data.get("offset", 0) if data else 0
                return await self.service.list_consumers(limit=limit, offset=offset)
            elif action == "delete":
                if consumer_id is None:
                    raise ValueError("consumer_id가 필요합니다")
                return await self.service.delete_consumer(consumer_id)
            else:
    ...
    async def _process_with_agent(
        ...
        if self.agent is None:
            self.agent = ConsumerAgent(adapter_path=self.adapter_path)
