...
export default function FileUploadPage() {
    ...
    const currentType: ItemType = 'schedule'
    ...
    const handleMenuClick = useCallback((itemType: ItemType) => {
        ...
        router.push(`/v10/admin/upload/${itemType}`)
    }, [router, currentType])
    ...
    const uploadFile = useCallback(async (fileItem: FileItem) => {
        ...
        try {
            ...
            // schedule 타입일 때는 soccer schedule router로 업로드 (포트 8000)
            const apiUrl = process.env.NEXT_PUBLIC_API_URL || 'http://localhost:8000'
            xhr.open('POST', `${apiUrl}/api/v10/soccer/schedule/upload`)
            xhr.send(formData)
